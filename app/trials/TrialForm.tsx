...
const TrialForm = ({errors, data, title, description}:{errors: PartialTrial; data?: ITrial; title?: string; description?: string}) => {
  ...
  > 
    <Input name="name" label="Название соревнования" errors={errors?.name} defaultValue={data?.name} col={2}/>
    ...
    <TextAria name="description" label="Описание" errors={errors?.description} defaultValue={data?.description} col={2}/>
  </Section>
  )
}
